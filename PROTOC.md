...
*   **Enum Aliases**:
    *   `Enum.values()` in Java excludes aliases (since they are just static fields pointing to constants).
    *   For Enums with aliases, `protoc` generates a helper method `getStaticValuesArray()` that manually constructs an array containing ALL values (including aliases) in definition order (matching `EnumDescriptor.getValues()`).
    *   The `VALUES` static field is initialized with this helper if aliases exist, otherwise with `values()`.
    *   A blank line is generated after `VALUES` definition ONLY if aliases are NOT present (i.e. using `values()`). If aliases are present, no blank line is generated after the `VALUES` field definition (because the helper method definition follows immediately?).
...
*   **Enum Field in Oneof Equals/HashCode**:
    *    was incorrectly accessing the oneof storage field directly (e.g., `oneofEnum_`) in `generateOneofEqualsCode` and `generateOneofHashCode`. It must use the public getter (`getOneofEnum()`) which handles the casting from the oneof storage object.
    *   For `equals`, it should use `!get...().equals(other.get...())` because getters return Enum objects (not ints) in Proto2.
    *   For `hashCode`, it should use `get...().getNumber()` to hash the integer value.

*   **Oneof Merging Indentation**:
    *   In `MessageBuilderGenerator`, the loop for merging oneofs prints the `case` label at 10 spaces indentation.
    *   The inner code generated by `generateMergingCode` in `EnumFieldGenerator` and `MessageFieldGenerator` was incorrectly indented at 10 spaces. It should be 12 spaces to match the context inside the `case` block.
    *   This was corrected to ensuring parity with standard `protoc` output which indents the merging logic.

*   **Enum Field Serialization**:
    *   In `writeTo` and `getSerializedSize` methods, generated code for regular Enum fields should NOT wrap the value variable in parentheses (e.g., `output.writeEnum(1, fieldName_);`).
    *   However, for Enum fields inside a oneof, the cast expression MUST be wrapped in parentheses (e.g., `output.writeEnum(1, ((java.lang.Integer) oneofField_));`). This matches `protoc` C++ implementation behavior.

*   **Enum Field Builder Override**:
    *   The `has...` method in the Builder for regular Enum fields (with presence) must be annotated with `@java.lang.Override`.
    *   To match `protoc` formatting exactly, the `@java.lang.Override` annotation and the method signature `public boolean has...` must appear on the **same line** for regular fields (e.g., `@java.lang.Override public boolean hasField() {`).

*   **Enum Map Field Handling**:
    *   Map fields with Enum values require generating additional `Value` accessors in the Interface, Message, and Builder (e.g., `get...Value`, `put...Value`).
    *   **Builder Order**: The `Value` accessors in the Builder are split. Getters (`get...Value`, `get...ValueMap`, etc.) must appear immediately after the standard `get...OrThrow` method. Mutators (`getMutable...Value`, `put...Value`, `putAll...Value`) must appear at the very end of the method (after standard mutators like `putAll`).
    *   **Javadoc**: The generated Javadoc for these methods must use `DocComment` utilities to generate the correct field definition string (wrapped in `<code>`), matching the format `map<KeyType, ValueType> name = number;`.
    *   **Null Checks**: Unlike Message map values, Enum map values in `put...` methods do NOT generate explicit null checks for the value parameter. The generator must suppress this check for Enums.
    *   **Formatting**: When `ValueNullCheck` is suppressed (for Enums), the standard generation logic that usually prints a blank line (if the check is missing) must be emulated. The `put...Value` generator explicitly adds a `printer.println()` after the key check to match this formatting behavior.

*   **Dependency Initialization**:
    *   The outer class generated by `FileGenerator` must explicitly initialize descriptors for all dependencies in its static initialization block.
    *   After calling `descriptor.resolveAllFeaturesImmutable()`, the code must loop through `file.getDependencies()` and call `DependencyClass.getDescriptor()` for each one. This ensures that dependencies are fully loaded and registered when the main class is accessed.

*   **Repeated Enum Fields - Value Accessors**:
