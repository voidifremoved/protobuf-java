...
public class PlatformDetector
{
	...
	public void detect(Properties props, List<String> classifierWithLikes)
	{
		...
		// For Linux systems, add additional properties regarding details of the
		// OS.
		...
		if (linuxRelease != null)
		{
			...
			// If any of the requested classifier likes are found in the "likes"
			// for this system,
	...
	private static LinuxRelease getLinuxRelease()
	{
		...
		// Older versions of redhat don't have /etc/os-release. In this case,
		// try
		// parsing this file.
	...
	/**
	 * Parses a file in the format of {@code /etc/os-release} and return a
	 * {@link LinuxRelease} based on the {@code ID}, {@code ID_LIKE}, and
	 * {@code VERSION_ID} entries.
	 */
	private static LinuxRelease parseLinuxOsReleaseFile(File file)
	{
		...
		{
			// Just absorb. Don't treat failure to read /etc/os-release as an
			// error.
		}
		finally
		{
	...
	/**
	 * Parses the {@code /etc/redhat-release} and returns a {@link LinuxRelease}
	 * containing the ID and like ["rhel", "fedora", ID]. Currently only
	 * supported for CentOS, Fedora, and RHEL. Other variants will return
	 * {@code null}.
	 */
