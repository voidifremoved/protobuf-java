...
public class MavenUtil
{
	...
	static URLHolder getReleaseDownloadUrl(String path, MavenSettings settings) throws IOException
	{
		...
		return new URLHolder(fullpath, settings.mProxyHost, settings.mProxyPort);
	}

	// get snapshot download URL
	static URLHolder getSnapshotDownloadUrl(String path, MavenSettings settings) throws IOException
	{
	...
	static String parseLastReleaseBuild(File mdFile, ProtocVersion protocVersion) throws IOException
	{
		...
		try
		{
			...
			for (int i = 0; i < versions.getLength(); i++)
			{
				...
				if (verStr.startsWith(protocVersion.version + "-build"))
				{
		...
		if (lastBuild > 0) return protocVersion.version + "-build" + lastBuild;
		return null;
	}
