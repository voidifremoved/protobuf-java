...
import com.rubberjam.protobuf.compiler.java.lite.ImmutableMapFieldLiteGenerator;
...
public class ImmutableMapFieldLiteGeneratorTest {
  ...
  public void testMapField() throws Exception {
    ...
    ImmutableMapFieldLiteGenerator generator =
        new ImmutableMapFieldLiteGenerator(field, 0, new Context(file, new Options()));

    generator.generateMembers(printer);

    ArgumentCaptor<Map<String, Object>> variablesCaptor = ArgumentCaptor.forClass(Map.class);
    // Verify Lite specific MapFieldLite usage
    verify(printer, atLeastOnce()).emit(variablesCaptor.capture(), contains("private com.google.protobuf.MapFieldLite<"));
    ...
    // Verify DefaultEntryHolder generation with MapEntryLite
    verify(printer, atLeastOnce()).emit(anyMap(), contains("static final com.google.protobuf.MapEntryLite<"));

    // Verify public accessor
    verify(printer, atLeastOnce()).emit(anyMap(), contains("public java.util.Map<$key_type$, $value_type$> ${$get$capitalized_name$Map$}$() {"));
  }
}
