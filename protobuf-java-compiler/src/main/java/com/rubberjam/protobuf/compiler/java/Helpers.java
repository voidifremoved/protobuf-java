...
public class Helpers {
	...
	public static boolean isRealOneof(FieldDescriptor field) {
		return field.getContainingOneof() != null
				&& !com.google.protobuf.CompilerInternalHelpers.isSynthetic(field.getContainingOneof());
	}
	...
	public static String defaultValue(FieldDescriptor field, boolean immutable, ClassNameResolver nameResolver,
			Options options) {
		switch (field.getType()) {
			case INT32:
			case SINT32:
	...
	public static boolean isByteStringWithCustomDefaultValue(FieldDescriptor field) {
		return field.getJavaType() == FieldDescriptor.JavaType.BYTE_STRING
				&& !field.getDefaultValue().equals(com.google.protobuf.ByteString.EMPTY);
	}
	...
	private static boolean hasRequiredFields(Descriptor descriptor, Set<Descriptor> visited) {
		...
		for (FieldDescriptor field : descriptor.getFields()) {
			...
			if (field.getType() == FieldDescriptor.Type.MESSAGE) {
				if (hasRequiredFields(field.getMessageType(), visited))
					return true;
			}
		}
		return false;
	...
	public static boolean bitfieldTracksMutability(FieldDescriptor field) {
		if (!field.isRepeated()
				|| (field.getType() == FieldDescriptor.Type.MESSAGE && isMapEntry(field.getMessageType()))) {
			return false;
		}
		switch (field.getType()) {
			case GROUP:
	...
	public static boolean isReferenceType(FieldDescriptor.JavaType type) {
		switch (type) {
			case INT:
			case LONG:
			case FLOAT:
