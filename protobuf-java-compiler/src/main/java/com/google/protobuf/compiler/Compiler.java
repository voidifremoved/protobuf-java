...
public final class Compiler {
  ...
  public interface ProtoImportResolver {
     ...
     * @param path The path of the proto file (e.g., "google/protobuf/descriptor.proto").
  ...
  public Map<String, String> compile(
        ...
        for (String dependency : proto.getDependencyList()) {
          if (!processed.contains(dependency)) {
            if (fileDescriptorProtos.containsKey(dependency)) {
               // Already parsed but not in processed set? Should not happen with logic above
               // unless passed in initial map but missed in toProcess init.
            ...
            // If content is null, we hope it's provided in the initial map or we fail later at buildFrom
          }
        }
      }
    }
    ...
    for (FileDescriptor fileDescriptor : fileDescriptors.values()) {
      // Only generate code for the requested files, not dependencies loaded via resolver
  ...
  private void parseProto(String fileName, String content, Map<String, FileDescriptorProto> outProtos) throws CompilationException {
