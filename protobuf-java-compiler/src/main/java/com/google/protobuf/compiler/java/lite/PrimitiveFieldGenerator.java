...
public class PrimitiveFieldGenerator extends ImmutableFieldGenerator {
  ...
  public void generateMembers(PrintWriter printer) {
    ...
    // Actually StringUtils.getJavaType returns JavaType enum, we need a helper to get the java type string
    // Let's assume a helper or map exists or just use a switch/case

    // Quick fix: assuming standard primitive types mapping for now or use descriptor.getJavaType().name().toLowerCase() but that might not match exactly (INT -> int is fine, but FLOAT -> float etc.)
    // Better: use descriptor.getJavaType() which returns Descriptors.FieldDescriptor.JavaType
    // and map it.
  ...
  private String getPrimitiveTypeName(FieldDescriptor descriptor) {
      switch (descriptor.getJavaType()) {
          ...
          case BYTE_STRING: return "com.google.protobuf.ByteString";
          case ENUM: return "int"; // Lite enums are often just ints or need special handling
          case MESSAGE: return ""; // Should not be here
