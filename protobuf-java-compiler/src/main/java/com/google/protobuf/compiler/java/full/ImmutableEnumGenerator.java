...
public class ImmutableEnumGenerator extends EnumGenerator {
  ...
  public void generate(PrintWriter printer) {
    ...
    for (Alias alias : aliases) {
        printer.println("  public static final " + classname + " " + alias.value.getName() + " = " + alias.canonicalValue.getName() + ";");
    }
    ...
    printer.println("    return " + context.getNameResolver().getFileClassName(descriptor.getFile(), true) + ".getDescriptor().getEnumTypes().get(" + descriptor.getIndex() + ");");
    printer.println("  }");
