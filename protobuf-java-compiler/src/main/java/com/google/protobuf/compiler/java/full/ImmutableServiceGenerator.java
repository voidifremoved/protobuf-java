...
public class ImmutableServiceGenerator extends ServiceGenerator {
  ...
  public void generate(PrintWriter printer) {
    ...
    printer.println("    return " + context.getNameResolver().getImmutableClassName(descriptor.getFile()) + ".getDescriptor().getServices().get(" + descriptor.getIndex() + ");");
    printer.println("  }");
  ...
  private void generateNewReflectiveServiceMethod(PrintWriter printer) {
    ...
    for (MethodDescriptor method : descriptor.getMethods()) {
        ...
        printer.println(" {");
        printer.println("        impl." + StringUtils.underscoresToCamelCase(method.getName(), false) + "(controller, request, done);");
        printer.println("      }");
    }
  ...
  private void generateMethodSignature(PrintWriter printer, MethodDescriptor method, boolean isAbstract) {
    ...
    printer.println("      public " + abstractKeyword + " void " + StringUtils.underscoresToCamelCase(method.getName(), false) + "(");
    ...
    printer.println("          com.google.protobuf.RpcCallback<" + context.getNameResolver().getImmutableClassName(method.getOutputType()) + "> done)");
  }
  ...
  private void generateCallMethod(PrintWriter printer) {
    ...
    for (MethodDescriptor method : descriptor.getMethods()) {
        ...
        printer.println("        this." + StringUtils.underscoresToCamelCase(method.getName(), false) + "(controller, (" + context.getNameResolver().getImmutableClassName(method.getInputType()) + ")request,");
        printer.println("          com.google.protobuf.RpcUtil.<" + context.getNameResolver().getImmutableClassName(method.getOutputType()) + ">specializeCallback(");
  ...
  private void generateCallBlockingMethod(PrintWriter printer) {
    ...
    for (MethodDescriptor method : descriptor.getMethods()) {
        printer.println("      case " + method.getIndex() + ":");
        printer.println("        return impl." + StringUtils.underscoresToCamelCase(method.getName(), false) + "(controller, (" + context.getNameResolver().getImmutableClassName(method.getInputType()) + ")request);");
    }
  ...
  private void generateBlockingMethodSignature(PrintWriter printer, MethodDescriptor method) {
    printer.println("    public " + context.getNameResolver().getImmutableClassName(method.getOutputType()) + " " + StringUtils.underscoresToCamelCase(method.getName(), false) + "(");
    printer.println("        com.google.protobuf.RpcController controller,");
    printer.println("        " + context.getNameResolver().getImmutableClassName(method.getInputType()) + " request)");
    printer.println("        throws com.google.protobuf.ServiceException");
  }
