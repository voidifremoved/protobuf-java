...
public class PrimitiveFieldGenerator extends ImmutableFieldGenerator {
  ...
  public static class RepeatedPrimitiveFieldGenerator extends ImmutableFieldGenerator {
    ...
    private void setPrimitiveVariables(
      ...
      String capitalizedType = StringUtils.toProperCase(variables.get("type")); // e.g. Int, Long
      if (javaType == JavaType.INT || javaType == JavaType.LONG || javaType == JavaType.BOOLEAN || javaType == JavaType.FLOAT || javaType == JavaType.DOUBLE) {
          ...
          // Fallback or byte string? For now primitive handles numeric/bool
          // C++ logic: else { ... Internal.ProtobufList<ByteString> ... }
          // This class is PrimitiveFieldGenerator so we expect numeric/bool.
      }
    ...
    @Override
    public int getNumBitsForBuilder() { return 0; } // Assuming simpler implementation without bitsets for now

    @Override
    public void generateInterfaceMembers(PrintWriter printer) {
      printer.println("  java.util.List<" + variables.get("boxed_type") + "> get" + variables.get("capitalized_name") + "List();");
    ...
    public void generateMembers(PrintWriter printer) {
      ...
      printer.println("  @java.lang.Override");
      printer.println("  public java.util.List<" + variables.get("boxed_type") + "> get" + variables.get("capitalized_name") + "List() {");
      printer.println("    return " + variables.get("name") + "_;");
      printer.println("  }");
    ...
    public void generateBuilderMembers(PrintWriter printer) {
      ...
      printer.println("  public Builder addAll" + variables.get("capitalized_name") + "(java.lang.Iterable<? extends " + variables.get("boxed_type") + "> values) {");
      printer.println("    ensure" + variables.get("capitalized_name") + "IsMutable();");
