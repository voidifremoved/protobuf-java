...
public class ImmutableExtensionGenerator extends ExtensionGenerator {
  ...
  public int generateNonNestedInitializationCode(PrintWriter printer) {
    ...
    if (descriptor.getExtensionScope() == null) {
        printer.println("    " + StringUtils.underscoresToCamelCase(descriptor.getName(), false) + ".internalInit(descriptor.getExtensions().get(" + descriptor.getIndex() + "));");
        bytecodeEstimate += 21;
    }
  ...
  public int generateRegistrationCode(PrintWriter printer) {
    printer.println("    registry.add(" + scope + "." + StringUtils.underscoresToCamelCase(descriptor.getName(), false) + ");");
    return 7;
  }

  private void initTemplateVars(FieldDescriptor descriptor, String scope, boolean immutable, Map<String, String> vars, Context context) {
